// Maçonnerie Girard — Philippe Girard — 2026-04-10
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const runCounter = (el) => {
    ...
    const timer = setInterval(() => { current = Math.min(current + increment, target); el.textContent = current + suffix; if (current >= target) clearInterval(timer); }, 20);
  };
  const checkStats = () => { statEls.forEach(el => { if (el.getBoundingClientRect().top < window.innerHeight - 60) runCounter(el); }); };
  ...
  document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', (e) => {
      ...
      if (!target) return;
      e.preventDefault();
      window.scrollTo({ top: target.getBoundingClientRect().top + window.scrollY - header.offsetHeight - 8, behavior: 'smooth' });
    });
  });
