// Plomberie Durand — Marc Durand — 2026-04-10
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ──────────────────────────────
     Contact form — urgence checkbox highlight
  ────────────────────────────── */
  ...
  if (urgenceCheck && urgenceRow) {
    urgenceCheck.addEventListener('change', () => {
      urgenceRow.classList.toggle('checked', urgenceCheck.checked);
    });
  }
  ...
  if (form && success) {
    form.addEventListener('submit', (e) => {
      ...
      required.forEach(field => {
        ...
        if (!field.value.trim()) { field.style.borderColor = '#E57373'; valid = false; }
      });
      if (!valid) return;
